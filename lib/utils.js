...
module.exports = function() { 
	...
	**/
	this.testDeVelocidad = function(){
		...
		// Command
		ssh.execCommand('speedtest --json').then(function(result) {
			console.log('STDOUT: ' + result.stdout);
			console.log('STDERR: ' + result.stderr);
	
			
			resolve({stdout: result.stdout, stderr: result.stderr, 
						ping: JSON.parse(result.stdout).ping, download: JSON.parse(result.stdout).download, upload: JSON.parse(result.stdout).upload});
			ssh.dispose();
		  })
		})});
	};
